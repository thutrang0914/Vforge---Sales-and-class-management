...
const stuId=(lead,course)=>lead.id+(COURSES.findIndex(c=>c.id===course)+1)/100; // id HV cố định theo lead + khóa: nhiều tab cùng tạo thì chỉ ghi 1 lần
...
function Crm({user,onLogout}){
  ...
  useEffect(()=>{
    ...
    toSync.forEach(l=>{
      ...
      if(b){
        ...
        newStudents.push({id:stuId(l,l.course),name:l.studentName,parentName:l.parentName,parentPhone:l.phone,course:l.course,classId:b.id,enrollDate:tod(),paymentStatus:"paid",amountPaid:leadFee(co,l,b),totalFee:leadFee(co,l,b),note:"Auto-sync"});
  ...
  return(<div>
    ...
    <tbody>{fl.map(l=>{const st=LEAD_ST.find(s=>s.id===l.status);const co=COURSES.find(c=>c.id===l.course);const isPaid=l.status==="paid"||l.status==="renew";const ac=classes.filter(c=>c.course===l.course&&["upcoming","active"].includes(c.status));const bc=bestCls(l.course);return<tr key={l.id} onMouseEnter={e=>e.currentTarget.style.background=V.surface2} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={{background:selL.has(l.id)?V.redDim:"transparent"}}>
      ...
      <TD><select value={l.course} onChange={e=>{const nc=e.target.value;const stu=l.status==="paid"&&students.find(s=>s.name===l.studentName&&s.course===l.course);if(stu){const nco=COURSES.find(c=>c.id===nc);const cur=classes.find(c=>c.id===stu.classId);const cls=cur?.course===nc?cur:(bestCls(nc)||cur);const fee=leadFee(nco,l,cls);setStudents(p=>p.map(s=>s.id===stu.id?{...s,course:nc,classId:cls?.id||s.classId,totalFee:fee,amountPaid:s.paymentStatus==="paid"?fee:s.amountPaid}:s));if(cls)setLeads(p=>p.map(x=>x.id===l.id?{...x,course:nc,assignedClass:cls.id}:x));else setLeads(p=>p.map(x=>x.id===l.id?{...x,course:nc}:x));log("Đổi khóa (đã đóng HP)",`${l.studentName}: ${co?.name} → ${nco?.name}`)}else setLeads(p=>p.map(x=>x.id===l.id?{...x,course:nc}:x))}} style={{padding:"4px 8px",background:`${gCC(co)}18`,border:`1px solid ${gCC(co)}44`,borderRadius:"6px",color:gCC(co),fontSize:"11px",fontWeight:700,outline:"none",cursor:"pointer"}}>{COURSE_LEVELS.map(lv=><optgroup key={lv.id} label={`${lv.icon} ${lv.name}`}>{COURSES.filter(c=>c.level===lv.id).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</optgroup>)}</select></TD>
  ...
  return(<div><h2 style={{color:V.text,margin:"0 0 24px",fontSize:"22px",fontWeight:800,fontFamily:"'Glory',sans-serif"}}>⚙️ <span style={{color:V.accent}}>Cài đặt</span></h2>
    <div style={{maxWidth:"600px"}}>
      ...
      <div style={{background:V.surface,border:`1px solid ${V.border}`,borderRadius:"14px",padding:"24px",marginTop:"20px"}}>
        ...
        <Btn variant="danger" onClick={()=>{if(confirm("XÓA SẠCH toàn bộ dữ liệu thật (lead, học viên, lớp, điểm danh)? Hành động KHÔNG thể hoàn tác!")){if(confirm("Xác nhận lần 2: bạn chắc chắn muốn xóa sạch?")){(async()=>{for(const[t,c]of[["attendance","id"],["students","id"],["leads","id"],["classes","id"],["audit_log","id"]]){const q=sb.from(t).delete();const{error}=await(t==="classes"?q.neq(c,""):q.gte(c,-1e300));if(error){alert("Lỗi xoá "+t+": "+error.message);return}}setLeads([]);setStudents([]);setClasses([]);setAttendance([]);setAuditLog([]);log("Xóa sạch toàn bộ dữ liệu","");alert("Đã xoá sạch dữ liệu trên Supabase.")})()}}}}>🗑 Xóa sạch dữ liệu</Btn>
      </div>
