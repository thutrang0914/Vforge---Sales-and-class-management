...
const ROLE_CFG={admin:{label:"Admin",color:V.vred,tabs:["dashboard","sales","classes","students","settings"]},sales:{label:"Sales",color:V.accent,tabs:["dashboard","sales","classes","students"]},reception:{label:"Lễ tân",color:V.purple,tabs:["dashboard","classes","students"]}};
...
export default function VforgeApp(){
  ...
  // NAV
  const allT=[{id:"dashboard",label:"Tổng quan",icon:<Ic.Dash/>},{id:"sales",label:"Sales",icon:<Ic.Sales/>},{id:"classes",label:"Lớp học",icon:<Ic.Class/>},{id:"students",label:"Học viên",icon:<Ic.Students/>},{id:"settings",label:"Cài đặt",icon:<Ic.Settings/>}];
  const visT=allT.filter(t=>can(t.id));
  const pg={dashboard:<Dash/>,sales:<SalesP/>,classes:<ClassP/>,students:<StuP/>,settings:<SetP/>};
